...
# example for detect "Skal" object function-------------------------------------------------
def detect4skal(hsv):
    global posX, posY

    # create mask image for detection of "Skal" object
    # --create mask image for detection of green area in "Skal" label
    msk_g1 = createMaskImage(hsv, [170, 180], [50, 120], [50, 240])
    msk_g2 = createMaskImage(hsv, [1, 5], [50, 120], [50, 240])
    createMaskImage(hsv, [1, 5], [50, 120], [50, 240])
    msk_g=msk_g1+msk_g2
    # --create mask image for detection of blue area in "Skal" label
    msk_b = createMaskImage(hsv, [110, 130], [0, 255], [50, 170])

    msk_r = createMaskImage(hsv, [1, 5], [50, 120], [50, 240])
    # --create mask image for detection of white area in "Skal" label
    msk_w = createMaskImage(hsv, [55, 90], [0, 50], [200, 255])
    # --create mask image for detection of "Skal" object by adding msk_g to msk_b
    mask = cv2.addWeighted(msk_g, 1, msk_b, 1, 0)
    mask = cv2.addWeighted(msk_w, 1, mask, 1, 0)
    ...
    for i in range(len(contours)):
        ...
        if w*1.5<h and h<w*3: # -- is "Skal"
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 2)#rect
            strSize = cv2.getTextSize("Pink", cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
            cv2.rectangle(img, (x, y-strSize[1]), (x+strSize[0], y), (0, 0, 255), -1)#textbround
            cv2.putText(img, "Pink", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))
        #else: # -- is not "Skal"
         #   cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)#color of rect
